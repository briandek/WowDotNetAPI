...
namespace Explorers.Test
{
	...
	public class RealmExplorerTest
	{
		...
		{
			IEnumerable<Realm> realmList = rE.GetAllRealms();
		...
		{
            IEnumerable<Realm> realmList = rE.Realms.WithType("pvp");
		...
		{
            IEnumerable<Realm> realmList = rE.Realms.ThatAreUp();
		...
		{
            IEnumerable<Realm> realmList = rE.Realms.WithoutQueues();
		...
		{
            IEnumerable<Realm> realmList = rE.Realms.WithPopulation("low");
		...
		{
            IEnumerable<Realm> realmList = rE.GetMultipleRealms("Skullcrusher", "Laughing Skull", "Blade's Edge");
		...
		{
            IEnumerable<Realm> realmList = rE.GetMultipleRealms("Blade's Edge", "AZUZU!", "dekuz");
		...
		public void Get_Realms_Using_Multiple_ValidNamesArray_Query_Returns_Valid_Results()
		{
            ...
            IEnumerable<Realm> realmList = rE.GetMultipleRealms(namesList);
		...
		{
            rE.Refresh();
            IEnumerable<Realm> realmList = rE.GetMultipleRealmsViaQuery("?realm=Medivh&realm=Blackrock");
		...
		{
            IEnumerable<Realm> realmList = rE.GetMultipleRealmsViaQuery("?realm=!!adfasdza...12");
			Assert.IsNotNull(realmList);
            Assert.IsTrue(realmList.Any());
		}
		...
		{
            IEnumerable<Realm> realmList = rE.GetMultipleRealmsViaQuery("?asdf2&asdf");
